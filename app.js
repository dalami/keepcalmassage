...
function setupSEO() {
  ...
  const schema = {
    ...
    address: {
      "@type": "PostalAddress",
      addressLocality: CONFIG.ubicacion || "",
    },
...
function buildProductos() {
  ...
  if (labelEl)
    labelEl.textContent = L("productosLabel", "Productos & Servicios");
  if (tituloEl) tituloEl.textContent = L("productosTitulo", "Lo que ofrecemos");
  if (ctaTexto)
    ctaTexto.textContent = L(
      "productosCTA",
      "¿Querés hacer un pedido o consultar disponibilidad?",
    );

  const waMsg = (nombre) =>
    `Hola ${CONFIG.nombre}! Me interesa: ${nombre}. ¿Podés darme más info?`;

  document.getElementById("productos-grid").innerHTML = CONFIG.productos
    .map(
      (p) => `
...
function buildTestimonios() {
  ...
  if (tituloEl)
    tituloEl.textContent = L(
      "testimoniosTitulo",
      "Lo que dicen nuestros clientes",
    );

  document.getElementById("testimonios-grid").innerHTML = CONFIG.testimonios
    .map(
      (t) => `
...
function buildContacto() {
  ...
  document.getElementById("contacto-desc").textContent = L(
    "contactoDesc",
    "Escribinos por WhatsApp o Instagram. Respondemos rápido.",
  );
...
function lightboxPrev(e) {
  e.stopPropagation();
  lightboxIndex =
    (lightboxIndex - 1 + CONFIG.galeria.length) % CONFIG.galeria.length;
  renderLightbox();
}
